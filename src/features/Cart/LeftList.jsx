...
export default function LeftList() {
  ...
  const totalInARow =
    cart &&
    cart.map((product) => {
      return product.quantity * product.Good.price;
    });

  const totalDevide = totalInARow && totalInARow.reduce((a, b) => a + b, 0);

  //totalDevide = cart.map((product) => {return product.quantity * product.Good.price;}).reduce((a, b) => a + b, 0);
  ...
  const onClickDeleteButtonHandler = (a) => {
    ...
    setCart(cart.filter((ddd) => ddd.cartsId !== a));
  };
        ...
        <Row1>
          <label>
            <input
              ...
              checked={
                cart && (checkItems.length == cart.length ? true : false)
              }
            />
            <span>전체선택</span>
          </label>
              ...
              <Products>
                {cart ? (
                  <div>
                        ...
                        <li key={abc.cartsId}>
                          <label>
                            <input
                              ...
                              checked={
                                checkItems.includes(abc.cartsId) ? true : false
                              }
                            />
                            <span></span>
                          </label>
                    ...
                    {/* <Temp>임시박스 최종금액 :{totalDevide}</Temp> */}
                  </div>
                ) : (
